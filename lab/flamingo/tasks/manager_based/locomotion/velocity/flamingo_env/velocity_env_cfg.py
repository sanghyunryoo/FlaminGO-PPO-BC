...
class ObservationsCfg:
    ...
    class NoneStackCriticCfg(ObsGroup):
        ...
        robot_joint_torque = ObsTerm(func=mdp.joint_torques)
        robot_joint_acc = ObsTerm(func=mdp.joint_acc)
        feet_lin_vel = ObsTerm(
            func=mdp.feet_lin_vel, params={"asset_cfg": SceneEntityCfg("robot", body_names=".*_wheel_link")}
        )
        robot_mass = ObsTerm(func=mdp.mass)
        robot_inertia = ObsTerm(func=mdp.inertia)
        robot_joint_stiffness = ObsTerm(func=mdp.joint_stiffness)
        robot_joint_damping = ObsTerm(func=mdp.joint_damping)
