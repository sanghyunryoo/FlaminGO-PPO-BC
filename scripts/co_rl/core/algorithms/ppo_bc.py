...
class PPO_BC:
    ...
    def _init_teacher(self):
        """
        Teacher Encoder + ActorCritic 초기화 및 체크포인트 로드
        """
        from scripts.co_rl.core.modules import ActorCritic, MLP_Encoder
        ...
        if isinstance(ckpt, dict):
            if "model_state_dict" in ckpt:
                ...
                state_enc = ckpt.get("encoder_state_dict", None)
            elif "model" in ckpt:
                state_ac = ckpt["model"]
        ...
        if state_enc is not None:
            enc_in_dim = state_enc["encoder.0.weight"].shape[1]   # 입력 obs 차원
            enc_hidden_dims = [w.shape[0] for k, w in state_enc.items() if "weight" in k][:-1]
            enc_out_dim = list(state_enc.values())[-1].shape[0]   # 마지막 layer 출력 크기
            print(f"[BC] Teacher encoder: in={enc_in_dim}, hidden={enc_hidden_dims}, out={enc_out_dim}")

            self.teacher_encoder = MLP_Encoder(
                num_input_dim=enc_in_dim,
                num_output_dim=enc_out_dim,
            ...
            missing_e, unexpected_e = self.teacher_encoder.load_state_dict(state_enc, strict=False)
            if missing_e:
                print(f"     missing encoder keys: {missing_e}")
            if unexpected_e:
                print(f"     unexpected encoder keys: {unexpected_e}")
        else:
    ...
    def update(self, expert_data=None, bc_coef=0.1):
        ...
        for (
            ...
            self.actor_critic.act(torch.cat((encoder_out_batch, obs_batch), dim=-1))
            ...
            if self.teacher is not None and self.bc_coef > 0.0:
                ...
                with torch.no_grad():
                    if self.teacher_encoder is not None:
                        t_enc = self.teacher_encoder.encode(teacher_obs_batch)
                        t_in  = torch.cat((t_enc, teacher_obs_batch), dim=-1)
                    else:
